...
protocol ArticleProviding {
    func fetchArticles(query: String, completionHandler: @escaping (Result<[Article]>) -> ())
}

class ArticleProvider: BasicProvider, ArticleProviding, RequestProcessor {
    func fetchArticles(query: String, completionHandler: @escaping (Result<[Article]>) -> ()) {
        var requestTarget = NYTRouter.getArticles(query: query, sort: SortType.newest).requestTarget
